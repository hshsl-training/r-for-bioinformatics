...
1. Search `Gene` for the BRCA1 gene in humans. What id is returned?
1. Use `entrez_link()` to find **all** databases that have a link to this Gene record. How many databases are linked? How many related records are there in Pubmed? 

:::: {.callout-tip title="Solution" icon=false collapse="true"}
1. `entrez_db_searchable("protein")`
1. `brca1_search <- entrez_search(db = "gene", term = "BRCA1[gene] AND human[orgn]")` <br>
  `brca1_search$ids`
1. `brca1_link <- entrez_link(dbfrom = "gene", id = test_search$ids, db = "all")`<br>
  `brca1_link$links`<br>
